...
export default function AIChatBox(){
  const [messages, setMessages] = useState<Message[]>(()=>{
    try{ return JSON.parse(localStorage.getItem('fb_chat')||'[]'); }catch{ return []; }
  });
    ...
    <section id="chat" className="container py-12">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Ask for code, content, or help with the builder. No keys required.</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-4">
              {messages.length === 0 && (
                <div className="text-sm text-muted-foreground">
                  Try: “Create a hero section with a headline, subtext and a button.”
                </div>
              )}
              {messages.map((m, i) => (
                <div key={i} className={`max-w-[85%] ${m.role==='user'?'ml-auto text-right':''}`}>
                  <div className={`inline-block rounded-2xl px-3 py-2 text-sm ${m.role==='user'?'bg-primary text-primary-foreground':'bg-muted'}`}>{m.content}</div>
                </div>
              ))}
              <div ref={bottomRef} />
            </div>
        ...
        <CardFooter className="gap-2">
          <Input value={input} onChange={(e)=>setInput(e.target.value)} placeholder="Ask anything..." onKeyDown={(e)=>{ if(e.key==='Enter'){ e.preventDefault(); send(); } }} />
