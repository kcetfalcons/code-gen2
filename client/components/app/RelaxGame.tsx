...
interface Orb { x: number; y: number; r: number; vx: number; vy: number; hue: number }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Calming motion. Click to gently nudge the orbs.</CardDescription>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={()=>setRunning(r=>!r)}>{running?"Pause":"Play"}</Button>
        ...
        <CardContent>
          <div className="rounded-xl overflow-hidden border bg-gradient-to-br from-secondary/40 to-accent/30">
            <canvas ref={canvasRef} onClick={onClick} className="block w-full h-[320px] md:h-[420px]"/>
          </div>
        </CardContent>
      </Card>
    </section>
...
function genOrbs(): Orb[]{
  ...
  for(let i=0;i<10;i++){
    ...
    out.push({ x: 80 + Math.random()*480, y: 80 + Math.random()*280, r, vx: (Math.random()-.5)*1.2, vy:(Math.random()-.5)*1.2, hue: 260 + Math.random()*60 });
  }
  return out;
}
