  ...
  try {
    ...
    if (!apiKey) {
      return res.status(200).json({
        provider: "local",
        message:
          localReply(messages?.[messages.length - 1]?.content || ""),
      });
    }
...
function localReply(text: string): string {
  ...
  if (t.includes("hero")) return "Add a Heading, Paragraph, and Button. Use H1 for the heading and strong contrast.";
  if (t.includes("nav")) return "Use a header with a logo at left and links at right. Keep it sticky and add a subtle border-bottom.";
  if (t.includes("grid")) return "Use a responsive grid: grid-cols-1 md:grid-cols-2 with gap-6 for balanced layout.";
  if (t.includes("form")) return "Use labels tied to inputs, add required and aria-invalid states, and group actions to the right.";
